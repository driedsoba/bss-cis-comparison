...
export default function BSSCISAnalyzer() {
  ...
  const processFiles = async (bssFile, cisFile) => {
    ...
    try {
      ...
      const sht = wb.SheetNames.find((n) => /settings|windows/i.test(n)) || wb.SheetNames[0];
      const raw = XLSX.utils.sheet_to_json(wb.Sheets[sht], { header: 1, defval: "" });
